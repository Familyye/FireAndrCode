...
import android.util.ReflectUtils;
...
public final class ActivityThread {
    ...
    //begin add fart by amlloc 2020.9.6
    public static void fart(){
        ...
        dexFile_fileField =  (Field) ReflectUtils.getClassField(appClassLoader, "dalvik.system.DexPathList$Element", "dexFile");
        try {
        ...
        for (Method method : DexFileClazz.getDeclaredMethods()){
            ...
            if (method.getName().equals("dumpMethodCode")){
                dumpMethodCodeMethod = method;
                defineClassMethod.setAccessible(true);
            }
        }

        Field mCookiefield = ReflectUtils.getClassField(appClassLoader, "dalvik.system.DexFile", "mCookie");
        for (int j = 0;j < dexElements.length; j++){
            Object dexElement = dexElements[j];
            Object dexFile = null;

            try {
                dexFile = dexFile_fileField.get(dexElement);
            ...
            if (dexFile != null){
                ...
                Object mCookie = ReflectUtils.getClassFieldObject(appClassLoader, "dalvik.system.DexFile", dexFile, "mCookie");
                if (mCookie ==  null){
                    continue;
                }
                ...
                if (classNames != null){
                    for (String eachClassName : classNames){
                        ReflectUtils.loadClassAndInvoke(appClassLoader, eachClassName, dumpMethodCodeMethod);
                    }
                }
            }
    ...
    public static void fartthread() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Log.e("amlloc", "ActivityThread start sleep,wait for fartthread start......");
                    Thread.sleep(1 * 60 * 1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                Log.e("amlloc", "ActivityThread sleep over and start fartthread");
                fart();
                Log.e("amlloc", "fart run over in ActivityThread ");
            }
        }).start();
    }
    //end add fart by amlloc 2020.9.6
