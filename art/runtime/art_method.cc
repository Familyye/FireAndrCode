...
//[start] added by amlloc in 2020.8.16
...
extern "C" void dumpDexFileByExecute(ArtMethod *artmethod)
  ...
  if (dexFilePath == nullptr) {
    LOG(INFO) << "ArtMethod::dumpDexFileByExecute,methodname:"
              << artmethod->PrettyMethod().c_str() << "malloc 2000 byte failed";
    return;
  }
  ...
  int procPid = getpid();
  sprintf(szCmdLine, "/proc/%d/cmdline", procPid);
  ...
  if (szProcName[0]) {
    ...
    const uint8_t *dexBegin = dexFile->Begin();
    size_t size_ = dexFile->Size();
    ...
    memset(dexFilePath, 0, 2000);
    sprintf(dexFilePath, "/sdcard/fart/%s/%d_dexFile_excute.dex", szProcName,
            size_int_);

    int dexFilefp = open(dexFilePath, O_RDONLY, 2000);
    if (dexFilefp > 0) {
      close(dexFilefp);
      dexFilefp = 0;
    } else {
      ...
      if (dexFilefp > 0) {
        if(write(dexFilefp, (void *)dexBegin, size_)){
            fsync(dexFilefp);
            close(dexFilefp);
        }
      }
...
extern "C" void dumpArtMethod(ArtMethod *artmethod) REQUIRES_SHARED(Locks::mutator_lock_) {
  ...
  LOG(INFO) << "ArtMethod::dumpArtMethodinvoked start ,methodname: " << artmethod->PrettyMethod().c_str(); 
  int fcmdline = -1;
  char szCmdline[64] = {0};
  char szProcName[256] = {0};
  int procid = getpid();
  ...
  if (szProcName[0]) {
    const DexFile *dexFile = artmethod->GetDexFile();
    const char *methodName = artmethod->PrettyMethod().c_str(); //获取方法名？
    const uint8_t *begin_ = dexFile->Begin();
    size_t size_ = dexFile->Size();
    ...
    memset(dexFilePath, 0, 2000);
    sprintf(dexFilePath, "/sdcard/fart/%s/%d_dexfile.dex", szProcName,
            size_int_);
    ...
    if (dexFilefp > 0) {
      ...
      dexFilefp = open(dexFilePath, O_CREAT | O_RDWR, 0666);
      if (dexFilefp > 0) {
        if(write(dexFilefp, (void *)begin_, size_) > 0){
            fsync(dexFilefp);
            close(dexFilefp);
        }
    ...
    char *method_massage = (char *)malloc(sizeof(char) * 2000); //用于存放method信息
      ...
      memset(method_massage, 0, 2000);
      int size_int = (int)dexFile->Size();
      uint32_t method_idx = artmethod->GetDexMethodIndex2();
      ...
      if (fp2 > 0) {
        ...
        sprintf(method_massage,
                "name:%s, method_idx:%d, offset:%d, code_item_len:%d, ins:",
                methodName, method_idx, offset, code_item_len);

        int contentLength = 0;
        while (method_massage[contentLength] != 0)
          contentLength++;

        int writeCount = -1;
        writeCount = write(fp2, (void *)method_massage, contentLength);
        long outLen = 0;
        char *codeItem_base64 =
            base64_encode((char *)item, (long)code_item_len, &outLen);
        writeCount = write(fp2, codeItem_base64, outLen);
        writeCount = write(fp2, "};", 2);
        fsync(fp2);
        close(fp2);
        if (codeItem_base64 != nullptr) {
          free(codeItem_base64);
          codeItem_base64 = nullptr;
        }
      }
	  }
  }
...
//[end] added by amlloc in 2020.8.16
...
void ArtMethod::Invoke(Thread* self, uint32_t* args, uint32_t args_size, JValue* result,
                       const char* shorty) {
  //[start] added by amlloc
  //当self线程为nullptr时，则判定是myfartInvoke所调用，此时执行dumpArtMethod
  if(self == nullptr){
    dumpArtMethod(this);
    return;
  }
